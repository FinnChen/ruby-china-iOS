...
#define INCREASE        @"increase"       // add by chenfeng
#define DATE_FORMAT_V2  @"date_format_v2" // add by chenfen
...
- (NSArray *)filters
{
	return [NSArray arrayWithObjects:
			INCREASE, DATE_FORMAT_V2, UPPERCASE, LOWERCASE, CAPITALIZED,
...
{
    if ([filter isEqualToString:INCREASE]) {
        if ([value isKindOfClass:[NSNumber class]]) {
            return [NSString stringWithFormat:@"%d", [(NSNumber *)value integerValue] + 1];
        }
    }  else if ([filter isEqualToString:DATE_FORMAT_V2]) {
		if ([value isKindOfClass:[NSDate class]]) {
			return [(NSDate *)value timeAgo];
		}
